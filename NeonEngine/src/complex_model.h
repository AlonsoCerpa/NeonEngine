...
public:
    Cylinder(GLfloat radius, GLfloat height, GLint sectorCount, GLint stackCount) {
        GLfloat x, y, z, xy;
        GLfloat sectorStep = 2 * M_PI / sectorCount;
        GLfloat heightStep = height / stackCount;
        GLfloat sectorAngle, heightCurr, radiusCurr;
        glm::vec3 vertex;

        for (int i = 0; i <= stackCount; ++i) {
            heightCurr = i * heightStep;
            xy = radius;
            radiusCurr = xy;

            for (int j = 0; j <= sectorCount; ++j) {
                sectorAngle = j * sectorStep;
                x = radiusCurr * cosf(sectorAngle);
                y = radiusCurr * sinf(sectorAngle);
                z = heightCurr;
                vertex.x = x;
                vertex.y = y;
                vertex.z = z;
        ...
        for (int j = 0; j <= sectorCount; ++j) {
            ...
            indices.push_back(j + stackCount * (sectorCount + 1));
        }
...
public:
    Cone(GLfloat radius, GLfloat height, GLint sectorCount, GLint stackCount) {
        GLfloat x, y, z, xy;
        GLfloat sectorStep = 2 * M_PI / sectorCount;
        GLfloat heightStep = height / stackCount;
        GLfloat sectorAngle, heightCurr, radiusCurr;
